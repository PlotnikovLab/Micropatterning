...
imagesize = 500 #Dimensions of images (number of pixels)
...
#Change this offset value (e.g., ±0.1) if thresholding does not work well
offset = 0
...
#saving each channel and merged image (assuming in the order of R-G-B, this can be changed by swapping the numbers 0~2 below)

image_rgb = np.zeros(image.shape[0]*image.shape[1]*3).reshape(image.shape[0],image.shape[1],3)

for a in range(image.shape[0]):
    for b in range(image.shape[1]):
        image_rgb[a,b,0] = image_shifted_all[a,b,0]
        image_rgb[a,b,1] = image_shifted_all[a,b,1]
        image_rgb[a,b,2] = image_shifted_all[a,b,2]
        
plt.imshow(image_rgb/65536)
plt.show()
...
imsave(path + '\\' + project_name + '_merge.tif', image_rgb.astype(np.uint16))
...
avg = np.zeros(3)
bg = np.zeros(3)
avgall = np.zeros(4*(samplesize+1)).reshape(4,samplesize+1)
bgall = np.zeros(4*(samplesize+1)).reshape(4,samplesize+1)
normalized = np.zeros(4*(samplesize+1)).reshape(4,samplesize+1)
